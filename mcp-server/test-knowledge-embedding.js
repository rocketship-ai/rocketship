...
async function runKnowledgeEmbeddingTest() {
  ...
  try {
    ...
    if (EMBEDDED_CLI_DATA?.version?.version) {
      log(
        GREEN,
        `   ✅ CLI Version detected: ${EMBEDDED_CLI_DATA.version.version}`
      );
    ...
    if (EMBEDDED_CLI_DATA?.help) {
      for (const cmd of expectedCommands) {
        if (EMBEDDED_CLI_DATA.help[cmd]) {
          ...
          if (
            EMBEDDED_CLI_DATA.help[cmd].help &&
            EMBEDDED_CLI_DATA.help[cmd].help.length > 50
          ) {
            log(
              GREEN,
              `   ✅ Command '${cmd}' has detailed help (${EMBEDDED_CLI_DATA.help[cmd].help.length} chars)`
            );
          }
        } else {
      ...
      log(
        GREEN,
        `   ✅ ${commandsFound}/${expectedCommands.length} expected commands found`
      );
    } else {
    ...
    if (
      EMBEDDED_CLI_DATA?.flags &&
      Object.keys(EMBEDDED_CLI_DATA.flags).length > 0
    ) {
    ...
    if (EMBEDDED_CLI_DATA?.installation) {
      ...
      if (installation.methods && installation.methods.length > 0) {
        log(
          GREEN,
          `   ✅ ${installation.methods.length} installation methods found`
        );
      ...
      if (installation.notAvailable && installation.notAvailable.length > 0) {
        log(
          GREEN,
          `   ✅ ${installation.notAvailable.length} unavailable methods documented`
        );
      ...
      if (installation.fromReadme) {
        log(
          GREEN,
          `   ✅ Installation info extracted from README (${installation.fromReadme.length} chars)`
        );
      }
    } else {
    ...
    if (EMBEDDED_CLI_DATA?.usage?.common_patterns) {
      ...
      const validatePatterns = patterns.filter((p) =>
      ...
      log(
        GREEN,
        `   ✅ ${validatePatterns.length} 'validate' command patterns`
      );

      // Check for correct flags
      const hasVarFile = patterns.some((p) => p.command.includes("--var-file"));
      const hasIncorrectVars = patterns.some((p) =>
        p.command.includes("--vars")
      );
      ...
      if (hasIncorrectVars) {
        log(
          RED,
          "   ❌ Incorrect --vars flag found in patterns (should be --var-file)"
        );
      } else {
    ...
    if (EMBEDDED_CLI_DATA?.usage?.file_structure) {
      ...
      if (structure.examples && Object.keys(structure.examples).length > 0) {
        log(
          GREEN,
          `   ✅ ${
            Object.keys(structure.examples).length
      ...
      if (
        ...
        Object.keys(structure.real_examples).length > 0
      ) {
        log(
          GREEN,
          `   ✅ ${
            Object.keys(structure.real_examples).length
          } real examples found in codebase`
        );
    ...
    if (EMBEDDED_CLI_DATA?.usage?.syntax_patterns?.variables) {
      ...
      if (vars.config && vars.config.length > 0) {
        log(
          GREEN,
          `   ✅ ${vars.config.length} config variable examples: ${vars.config
            .slice(0, 3)
            .join(", ")}`
        );
      }
      ...
      if (vars.environment && vars.environment.length > 0) {
        log(
          GREEN,
          `   ✅ ${
            vars.environment.length
          } environment variable examples: ${vars.environment
            .slice(0, 3)
            .join(", ")}`
        );
      }
      ...
      if (vars.runtime && vars.runtime.length > 0) {
        log(
          ...
          `   ✅ ${
            vars.runtime.length
          } runtime variable examples: ${vars.runtime.slice(0, 3).join(", ")}`
        );
      }
    } else {
    ...
    if (EMBEDDED_SCHEMA) {
      ...
      if (
        EMBEDDED_SCHEMA.properties?.tests?.items?.properties?.steps?.items
        ...
        log(
          GREEN,
          `   ✅ ${plugins.length} plugins in schema: ${plugins.join(", ")}`
        );
    ...
    if (EMBEDDED_EXAMPLES && EMBEDDED_EXAMPLES.size > 0) {
      ...
      log(
        GREEN,
        `   ✅ Example types: ${exampleNames.slice(0, 5).join(", ")}${
          exampleNames.length > 5 ? "..." : ""
        }`
      );
    ...
    if (EMBEDDED_DOCS && EMBEDDED_DOCS.size > 0) {
      ...
      log(
        GREEN,
        `   ✅ Doc types: ${docPaths.slice(0, 5).join(", ")}${
          docPaths.length > 5 ? "..." : ""
        }`
      );
    } else {
    ...
    if (EMBEDDED_CLI_DATA?.installation?.notAvailable) {
      const brewNotAvailable = EMBEDDED_CLI_DATA.installation.notAvailable.some(
        (na) =>
          na.toLowerCase().includes("homebrew") ||
          na.toLowerCase().includes("brew")
      );
      if (brewNotAvailable) {
        log(
          GREEN,
          "   ✅ Homebrew correctly marked as unavailable (fixes brew upgrade issue)"
        );
    ...
    if (
      EMBEDDED_CLI_DATA?.version?.version &&
      EMBEDDED_CLI_DATA.version.version.includes("0.5.6")
    ) {
    ...
    const testResults = [
      ...
      { name: "Schema Loading", passed: !!EMBEDDED_SCHEMA },
      {
        ...
        passed: EMBEDDED_EXAMPLES && EMBEDDED_EXAMPLES.size > 0,
      },
      {
        name: "Documentation Loading",
        passed: EMBEDDED_DOCS && EMBEDDED_DOCS.size > 0,
      },
    ];
    ...
    if (passedTests === totalTests) {
      log(
        GREEN,
        `🎉 All ${totalTests} tests passed! Knowledge embedding is working perfectly.`
      );
    } else {
      log(
        YELLOW,
        `⚠️  ${passedTests}/${totalTests} tests passed. Some issues may need attention.`
      );
    }
