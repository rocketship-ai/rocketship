...
import { usePendingProjectInvites, useAcceptProjectInvite, useProjectInvitePreview } from '../hooks/use-console-queries';
...
export function AcceptInvite({ onComplete }: AcceptInviteProps) {
  ...
  const { data: pendingInvites = [], isLoading: invitesLoading, error: invitesError } = usePendingProjectInvites();
  const { data: previewInvite, isLoading: previewLoading, error: previewError } =
    useProjectInvitePreview(inviteId || null, code || null, { enabled: !!inviteId && !!code });
  ...
  const handleAccept = async () => {
    ...
    if (!inviteId.trim()) {
      setError('Invite link is missing. Please use the invite link from your email.');
      return;
    }

    setError(null);
    try {
      const result = await acceptMutation.mutateAsync({ invite_id: inviteId.trim(), code: code.trim() });
      ...
      <div className="min-h-screen bg-[#fafafa] flex items-center justify-center p-4">
        <div className="bg-white rounded-lg border border-[#e5e5e5] shadow-sm p-8 max-w-md w-full text-center">
          ...
          <div className="bg-[#fafafa] rounded-md p-4 mb-6 text-left">
            <p className="text-sm text-[#666666] mb-2">Projects you now have access to:</p>
            <ul className="space-y-1">
              {acceptedInvite.projects.map((p) => (
                <li key={p.project_id} className="flex items-center justify-between text-sm">
                  <span>{p.project_name}</span>
                  <span className={`px-2 py-0.5 rounded-full text-xs ${
                    p.role === 'write' ? 'bg-green-50 text-green-700' : 'bg-blue-50 text-blue-700'
                  }`}>
                    {p.role}
                  </span>
                </li>
      ...
      <div className="bg-white rounded-lg border border-[#e5e5e5] shadow-sm p-8 max-w-lg w-full">
        ...
        {invitesError ? (
          <div className="flex items-center gap-2 py-4 mb-6 px-3 bg-red-50 rounded-md border border-red-200">
            ...
            <span className="text-sm text-red-700">
              Failed to load pending invites: {invitesError instanceof Error ? invitesError.message : 'Unknown error'}
            </span>
          </div>
        ) : previewError ? (
          <div className="flex items-center gap-2 py-4 mb-6 px-3 bg-red-50 rounded-md border border-red-200">
            <AlertCircle className="w-5 h-5 text-red-600 flex-shrink-0" />
            <span className="text-sm text-red-700">
              Failed to load invite: {previewError instanceof Error ? previewError.message : 'Unknown error'}
            </span>
          </div>
        ) : invitesLoading || previewLoading ? (
          <div className="flex items-center justify-center py-4 mb-6">
            <Loader2 className="w-5 h-5 animate-spin text-[#666666]" />
            <span className="ml-2 text-sm text-[#666666]">Checking for pending invites...</span>
          </div>
        ) : previewInvite ? (
          <div className="mb-6">
            ...
            <div className="p-3 bg-[#fafafa] rounded-md border border-[#e5e5e5]">
              ...
              <div className="mt-2 flex flex-wrap gap-1">
                {previewInvite.projects.map((p) => (
                  ...
                  >
                    {p.project_name}
                    <span className={`font-medium ${p.role === 'write' ? 'text-green-600' : 'text-blue-600'}`}>
                      ({p.role})
                    </span>
                  </span>
                ))}
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={acceptMutation.isPending || !code.trim() || !inviteId.trim()}
