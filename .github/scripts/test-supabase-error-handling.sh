...
if [ -f ".rocketship-errors/supabase-error-handling.env" ]; then
    OUTPUT=$(rocketship run -af .rocketship-errors/supabase-error-handling.yaml --env-file .rocketship-errors/supabase-error-handling.env 2>&1 || true)
else
    OUTPUT=$(rocketship run -af .rocketship-errors/supabase-error-handling.yaml 2>&1 || true)
fi
